...
def testlist():
    frm = Question('Що означає ключове слово "elif" y Python?', 'інакше, якщо', 'інакше', 'якщо', 'умова')
    questions_listmodel.form_list.append(frm)
    frm = Question('Яка функція використовується для виведення даних на екран y Python?', 'print', 'output', 'echo', 'write') 
    questions_listmodel.form_list.append(frm)
    frm = Question('Кіт', 'cat', 'cut', 'cart', 'coat') 
    questions_listmodel.form_list.append(frm)
    frm = Question('Що повертає функція len() y Python?', 'довжину об’єкта', 'перший елемент', 'останній елемент', 'тип об’єкта') 
    questions_listmodel.form_list.append(frm)
